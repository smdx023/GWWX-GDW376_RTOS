...
/**
  ...
  * @retval 
  * 
  * RCC->CR(时钟控制寄存器) |= (uint32_t)0x00000001;   bit0 = 1内部8MHz振荡器(HSION)开启
  * RCC->CFGR(时钟配置寄存器) &= (uint32_t)0xF0FF0000; 
  *   --bit1:0 系统时钟切换 (System clock switch) = 00内部8MHz振荡器(HSION)作为系统时钟
  *   --bit7:4 HPRE(AHB预分频)
  */
...
#else
  RCC->CFGR &= (uint32_t)0xF0FF0000; //1:0=00->内部8MHz振荡器(HSION)作为系统时钟
#endif /* STM32F10X_CL */   
